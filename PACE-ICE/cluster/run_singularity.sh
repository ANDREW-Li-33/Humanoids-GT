...
singularity exec \
    ...
    -B /etc/hosts:/etc/hosts:ro \
    --nv --writable --containall $TMPDIR/$2.sif \
    bash -c "export PYTHONUNBUFFERED=1 && export ISAACLAB_PATH=/workspace/isaaclab && cd /workspace/isaaclab && /isaac-sim/python.sh ${CLUSTER_PYTHON_EXECUTABLE} ${@:3}"
